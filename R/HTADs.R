...
#' @param chrom `character`. The name of the chromosome.
...
#' save_bed_coordinates(htads, chrom = 'chr18', file = 'chr18_borders.bed')
#' @export

save_bed_coordinates <- function(htads, chrom, file = '') {
    start <- htads$clusters[[as.character(htads$optimal_n_clusters)]]$coord$start
    end <- htads$clusters[[as.character(htads$optimal_n_clusters)]]$coord$end
    write.table(cbind(chrom, start, end),
                file = file,
                quote = FALSE,
                sep = '\t',
                row.names = FALSE,
